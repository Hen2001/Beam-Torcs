...
static void rmGraniteAnalysis(void *prevHdle)
{
    ...
    while (linePtr && y > 80)
    {
        ...
        while (strlen(tempLine) > maxCharsPerLine)
        {
            ...
            GfuiLabelCreate(analysisScr, segment, GFUI_FONT_MEDIUM_C,
                            320, y, GFUI_ALIGN_HC_VB, 0);
            y -= 20;

            memmove(tempLine, tempLine + breakPos + 1,
                    strlen(tempLine) - breakPos);
        }

        if (strlen(tempLine) > 0) {
            GfuiLabelCreate(analysisScr, tempLine, GFUI_FONT_MEDIUM_C,
                            320, y, GFUI_ALIGN_HC_VB, 0);
            y -= 22;
        }
